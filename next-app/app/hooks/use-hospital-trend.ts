import { dayNames, WeekDayNames } from "@/app/lib/types";
...
const getHospitalTrend = async (
	...
	const endpoint = `/api/hospitals/${id}/trend`;
	const params = new URLSearchParams();
	if (day) params.append("day", day);
	if (day && hour !== undefined) params.append("hour", hour.toString());
	const response = await axios.get<GetHospitalTrendResponse>(endpoint, {
		params,
	});
...
export const usePrefetchHospitalTrend = (queryClient: QueryClient) => {
	const prefetchTrendData = (id: number) => {
		const now = new Date();
