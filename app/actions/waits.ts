 ...
 */
const parseManagementStatus = (
    status: "Y" | "N" | "N/A" | string
): ManagementStatus => {
    switch (status) {
        case "Y":
...
export async function getWaitTimes(): Promise<ApiResponse> {
    try {
        ...
        if (!rawData || !Array.isArray(rawData.waitTime)) {
            throw new Error(
                "Invalid data structure received from the external API."
            )
        }

        const transformedData: TransformedHospitalData[] = rawData.waitTime.map(
            (hospital) => {
                ...
                return {
                    ...
                    criticalManagementStatus: parseManagementStatus(
                        hospital.manageT1case
                    ),
                    emergencyManagementStatus: parseManagementStatus(
                        hospital.manageT2case
                    ),
                    waitTimes: {
                        ...
                        semiUrgentNonUrgentP50Minutes: parseWaitTime(
                            hospital.t45p50
                        ),
                        semiUrgentNonUrgentP95Minutes: parseWaitTime(
                            hospital.t45p95
                        ),
                    },
                }
