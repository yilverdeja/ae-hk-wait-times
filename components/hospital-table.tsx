...
import { usePrefetchHospitalTrend } from "@/hooks/useHospitalTrend";
...
export default function HospitalTable({
  ...
  useEffect(() => {
    if (data && table.getRowModel().rows.length > 0) {
      table
        .getRowModel()
        .rows.slice(0, 5)
        .forEach((row) => {
          usePrefetchHospitalTrend(queryClient, row.original.id);
        });
    }
  }, [table.getRowModel().rows, data]);

  const handleHospitalClick = (hospital: HospitalWithWait, index: number) => {
    ...
    const selectedRowIndex = sortedRows.findIndex((row) => row.index === index);
          ...
          : table.getRowModel().rows.map((row) => {
              return (
                <TableRow
                  ...
                  onClick={() => handleHospitalClick(row.original, row.index)}
                  onMouseEnter={() =>
                    usePrefetchHospitalTrend(queryClient, row.original.id)
                  }
                >
