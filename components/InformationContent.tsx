...
export const informationContent: InformationContent = {
    ...
    accordionItems: [
        ...
        },
        {
            ...
            title: {
                en: "How Are Wait Times Predicted?",
                zh: "如何預測等候時間？",
                cn: "如何预测等候时间？",
            },
            content: {
                en: (
                    <>
                        <p className="mb-2">
                            Predicted wait times are generated by a machine
                            learning model trained on approximately{" "}
                            <strong>8 months of historical data</strong>.
                            Predictions appear on the trend chart at +1h, +2h,
                            and +3h intervals from the current time.
                        </p>
                        <p className="mb-2">
                            <strong>The model is not perfect.</strong> It tends
                            to underpredict at higher wait times, so forecasts
                            are automatically raised to at least the current
                            wait when times exceed {PREDICTION_CAP_MINS / 60}{" "}
                            hours, and predictions are hidden entirely beyond{" "}
                            {PREDICTION_SUPPRESS_MINS / 60} hours.
                        </p>
                        <p>
                            Predictions are{" "}
                            <strong>for reference only</strong> and do not
                            constitute medical advice. Use them at your own
                            discretion — actual conditions can change rapidly.
                            We are continuously reviewing and improving the
                            model.
                        </p>
                    </>
                ),
                zh: (
                    <>
                        <p className="mb-2">
                            預測等候時間由機器學習模型生成，該模型以約
                            <strong>8個月的歷史數據</strong>
                            進行訓練。預測顯示於趨勢圖，分別為當前時間後1小時、2小時及3小時。
                        </p>
                        <p className="mb-2">
                            <strong>模型並非完美。</strong>
                            在等候時間較長時，模型可能低估預測值，因此當等候時間超過{" "}
                            {PREDICTION_CAP_MINS / 60}{" "}
                            小時時，預測值會自動調高至不低於當前等候時間；超過{" "}
                            {PREDICTION_SUPPRESS_MINS / 60}{" "}
                            小時時，預測將完全隱藏。
                        </p>
                        <p>
                            預測數據<strong>僅供參考</strong>
                            ，不構成醫療建議。請自行斟酌使用——實際情況可能迅速變化。我們將持續檢討並改善模型。
                        </p>
                    </>
                ),
                cn: (
                    <>
                        <p className="mb-2">
                            预测等候时间由机器学习模型生成，该模型以约
                            <strong>8个月的历史数据</strong>
                            进行训练。预测显示于趋势图，分别为当前时间后1小时、2小时及3小时。
                        </p>
                        <p className="mb-2">
                            <strong>模型并非完美。</strong>
                            在等候时间较长时，模型可能低估预测值，因此当等候时间超过{" "}
                            {PREDICTION_CAP_MINS / 60}{" "}
                            小时时，预测值会自动调高至不低于当前等候时间；超过{" "}
                            {PREDICTION_SUPPRESS_MINS / 60}{" "}
                            小时时，预测将完全隐藏。
                        </p>
                        <p>
                            预测数据<strong>仅供参考</strong>
                            ，不构成医疗建议。请自行斟酌使用——实际情况可能迅速变化。我们将持续检讨并改善模型。
                        </p>
                    </>
                ),
            },
